...
class OAuth2ServiceCallFilter @Inject() (oauth2: OAuth2Helper) extends Filter
{
  ...
  override def apply(nextFilter: RequestHeader => Future[Result]) 
    ...
    if (authHeader == NO_AUTH_HEADER) {
      ...
      return Future[Result](Results.BadRequest("no authorization header provided in request"))
    }
    ...
    if (accessToken == NO_TOKEN) {
      ...
      return Future[Result](Results.BadRequest("Authorization header does not contain any access token"))
    }
    ...
    if (! isAccessGranted(tokenInfoResponse)) {
      ...
      return Future[Result](Results.Forbidden("access is not granted"))
    }
